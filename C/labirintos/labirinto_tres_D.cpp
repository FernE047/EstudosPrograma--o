...
const int base=((LARGURA-1)/2)*((ALTURA-1)/2)*((PROFUNDIDADE-1)/2);  // pseudo-constantes
short int x,y,z,direcao,x_curvas[base],y_curvas[base],z_curvas[base],himym,teste_de_cont; //criação do labirinto
short int lab[LARGURA+1][ALTURA][PROFUNDIDADE],x_fim,y_fim,z_fim;			//labirinto manipulavel
...
int zescreve(){
    ...
    for(y_escreve=0;y_escreve<ALTURA;y_escreve++)
    {
    	for(x_escreve=1;x_escreve<(LARGURA+1);x_escreve++)
    	{
        	...
        	printf("%c",lab[x_escreve][y_escreve][z]);
    	}
    }
    if(z==1){
    	mgotoxy(2,1);
...
int main(){
    ...
    while(himym==1){
    	if((x_curvas[controle_de_base]>1)and(y_curvas[controle_de_base]>1)and(z_curvas[controle_de_base]>1)){
    		himym=0;
