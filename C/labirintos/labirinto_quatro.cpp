...
const int base=((LARGURA-1)/2)*((ALTURA-1)/2);  // pseudo-constantes
short int x,y,direcao,x_curvas[base],y_curvas[base],himym,teste_de_cont; //criação do labirinto
short int lab[LARGURA+1][ALTURA],x_fim,y_fim;			//labirinto manipulavel
char tecla;
...
int sera(){
	if ((lab[x+2][y]==0)or(x==LARGURA-1)){
		if((lab[x-2][y]==0)or(x==2)){
			if((lab[x][y+2]==0)or(y==ALTURA-2)){
